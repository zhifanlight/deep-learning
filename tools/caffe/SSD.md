...
- PriorBox 层处理流程：
	...
	- 生成 \\(ratio = 1:1, \quad height = min, \quad width = min\\) 的 Default Box

	- 生成 \\(ratio = 1:1, \quad height = \sqrt{min \cdot max}, \quad width = \sqrt{min \cdot max}\\) 的 Default Box

	- 根据 ```aspect_ratio``` 和 ```flip``` 参数，生成其他的 Default Box

	- 记录 ```variance``` ，用于计算 loss
...
- DetectionOutput 层处理流程：

	- 根据 Default Box 和 BBox 变换量，得到原图上的目标位置（百分比）

	- 保留置信度最高的 ```top_k``` 个目标，之后进行 NMS

	- 如果目标过多，保留置信度最高的 ```keep_top_k``` 个目标

## 网络输出

- BBox 输出 ```mbox_loc``` 维度为 \\((1, \ 4D)\\)

	- \\(D\\) 表示所有特征图上所有 Default Box 数量

- Softmax 输出 ```mbox_conf_flatten``` 维度为 \\((1, \ DC)\\)

	- \\(C\\) 表示分类的类别数

- PriorBox 层输出 ```mbox_priorbox ``` 维度为 \\((1, \ 2, \ 4D)\\)

	- 第一个通道存储每个 Default Box 在原图上的位置 \\(x1, \ y1, \ x2, \ y2\\)

	- 第二个通道存储对应的 ```variance```，用于计算 loss

- 最终输出 ```detection_out``` 维度为 \\((1, \ 1, \ K, \ 7)\\)

	- \\(K\\) 是每张图上检测到的目标数量

	- \\(7\\) 维向量分别是 \\(id, \ class, \ conf, \ x1, \ y1, \ x2, \ y2\\)
