...
- 假设决策树有 \\(|T|\\) 个叶结点，每个叶结点中有 \\(N\_{t}\\) 个样本，其中第 \\(k\\) 类样本有 \\(N\_{tk}\\) 个，损失函数计算如下：

	$$ C\_{\alpha}(T) = \sum\_{t=1}^{|T|} N\_{t} H\_{t}(T) + \alpha |T| $$

	- 其中 \\(H\_{t}(T)\\) 为第 \\(t\\) 个结点所有样本的经验熵，\\(N\_{t}\\) 相当于样本数量加权：

		$$ H\_{t}(T) = -\sum\_{k} \frac{N\_{tk}}{N\_{t}} \log \frac{N\_{tk}}{N\_{t}} $$
